...
import (
	...
	pb "crawlab/core/registry/transport/grpc/proto"
)
...
func ToProtos(s []*registry.Service) []*pb.Service{
	var services = make([]*pb.Service, len(s))
	for _,srv:=range s{
		services  = append(services, ToProto(srv))
	}
	return services
}
