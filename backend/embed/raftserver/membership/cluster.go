...
func NewClusterFromURLsMap(lg log.Interface, token string, urlsmap types.URLsMap) (*RaftCluster, error) {
...
func NewClusterFromMembers(lg log.Interface, token string, id types.ID, membs []*Member) *RaftCluster {
	c := NewCluster(lg, token)
	c.cid = id
	for _, m := range membs {
		c.members[m.ID] = m
...
func NewCluster(lg log.Interface, token string) *RaftCluster {
	if lg == nil {
		lg = log.Log
	}
	return &RaftCluster{
...
func (c *RaftCluster) MemberByName(name string) *Member {
	...
	for _, m := range c.members {
		if m.Name == name {
			if memb != nil {
				c.lg.Errorf("%s: two member with same name found", name)
			}
			memb = m
		}
	}
